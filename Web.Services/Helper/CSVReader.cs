...
namespace Web.Services.Helper
{
    public class CSVReader
    {
        ...
        private DataTable CreateDataTable(string[] DataColumns)
        {
            DataTable tbl = new DataTable();
            foreach (string strColumn in DataColumns)
            {
                DataColumn col = new DataColumn(strColumn.Trim().ToUpper(), typeof(string));
                tbl.Columns.Add(col);
            }
            return tbl;
        }
        public void WriteDataTableAsCSV(DataTable dt, string path)
        {
        ...
        public DataTable GetCSVAsDataTable(string path)
        {
            ...
            using (StreamReader streamReader = new StreamReader(fileStream))
            {
                while (true)
                {
                    ...
                    string[] columns = LineToArray(line);
                    if (count == 0)
                    {
        ...
        #region private code of csv reader to handle csv file
